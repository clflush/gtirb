...
(cl:defmethod cl:initialize-instance :after
  ((self image-byte-map) cl:&key
                  (uuid cl:nil uuid-p)
                  (byte-map cl:nil byte-map-p)
                  (addr-min cl:nil addr-min-p)
                  (addr-max cl:nil addr-max-p)
                  (base-address cl:nil base-address-p)
                  (entry-point-address cl:nil entry-point-address-p)
                  )
  (cl:declare (cl:ignore uuid byte-map addr-min addr-max base-address entry-point-address))
  (cl:when uuid-p
